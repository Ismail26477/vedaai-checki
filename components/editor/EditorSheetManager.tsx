...
export function EditorSheetManager() {
  ...
  const handleCreateSheet = async () => {
    ...
    try {
      ...
      if (response.success && response.sheet) {
        ...
        if (newSheet.id) {
          console.log("[v0] Navigating to sheet:", newSheet.id)
          router.push(`/editor/sheets/${newSheet.id}`)
        } else {
          console.error("[v0] Sheet created but no ID returned:", newSheet)
        }
      } else {
  ...
  const handleOpenSheet = (sheetId: string) => {
    ...
    router.push(`/editor/sheets/${sheetId}`)
  }
